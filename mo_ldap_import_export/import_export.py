...
class SyncTool:
    ...
    async def mo_org_unit_address_to_ldap(
        ...
        for address_type, addresses in address_map.items():
            ...
            if ldap_object_class != employee_object_class:
                logger.warning(
                    "Mapping organization unit addresses to non-employee objects is not supported"
                )
                continue
    ...
    async def refresh_people_by_org_unit(self, uuid: OrgUnitUUID) -> None:
        """Synchronize employees with an engagement to the org-unit.

        Args:
            uuid: UUID of the changed org-unit.
        """
        await self.dataloader.graphql_client.org_unit_engagement_people_refresh(
            self.amqpsystem.exchange_name, uuid
        )
