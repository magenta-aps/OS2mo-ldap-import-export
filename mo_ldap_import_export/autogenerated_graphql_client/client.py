...
class GraphQLClient(AsyncBaseClient):
    ...
    async def read_itsystem_uuid(self, user_key: str) -> ReadItsystemUuidItsystems:
        query = gql(
            ...
            query read_itsystem_uuid($user_key: String!) {
              itsystems(filter: {user_key: [$user_key]}) {
                objects {
                  uuid
                }
              }
    ...
    async def read_itsystem_user_key(self, uuid: UUID) -> ReadItsystemUserKeyItsystems:
        query = gql(
            """
            query read_itsystem_user_key($uuid: UUID!) {
        ...
        response = await self.execute(query=query, variables=variables)
        data = self.get_data(response)
        return ReadItsystemUserKey.parse_obj(data).itsystems
