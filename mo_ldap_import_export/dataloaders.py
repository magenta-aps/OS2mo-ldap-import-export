...
def employee2graphql_create(obj: Employee) -> EmployeeCreateInput:
    """Converts an RAModels Employee to a GraphQL EmployeeCreateInput.

    Raises:
        AssertionError: If the conversion is not possible.

    Args:
        obj: The RAModels employee to be converted.

    Returns:
        The 'equivalent' GraphQL EmployeeCreateInput.
    """
    assert obj.type_ == "employee"
    # Required fields
    assert obj.givenname is not None
    assert obj.surname is not None
    # Unhandled fields must be empty
    assert obj.org is None
    assert obj.name is None
    assert obj.nickname is None
    assert obj.details is None or obj.details == []
...
class DataLoader:
    ...
    async def create_employee(self, obj: Employee) -> UUID:
        result = await self.graphql_client.employee_create(employee2graphql_create(obj))
        return result.uuid

    async def create_address(self, obj: Address) -> Any:
        # TODO: Ensure robustness of this mapping
        ra_dictionary = obj.dict()
        ra_dictionary["validity"]["from"] = ra_dictionary["validity"].pop("from_date")
        ra_dictionary["validity"]["to"] = ra_dictionary["validity"].pop("to_date")
        ...
        def strip_uuid_obj(key, value) -> Any:
        ...
        ra_dictionary = {
            key: strip_uuid_obj(key, value) for key, value in ra_dictionary.items()
        }

        result = await self.graphql_client.address_create(
            AddressCreateInput(**ra_dictionary)
        )
        return result.uuid
